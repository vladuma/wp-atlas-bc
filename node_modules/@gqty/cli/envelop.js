...
const promise = require('gqty/Utils/promise');
...
function useGenerateGQty(config) {
  const pluginDeps = promise.LazyPromise(async () => {
    const [{ writeGenerate }, { config: gqtyConfig }, { defaultConfig }] = await Promise.all([
      Promise.resolve().then(function () { return require('./writeGenerate.js'); }),
      Promise.resolve().then(function () { return require('./config.js'); }).then((v) => v.gqtyConfigPromise),
      Promise.resolve().then(function () { return require('./config.js'); })
    ]);
    return {
      writeGenerate,
      gqtyConfig,
...
exports.useGenerateGQty = useGenerateGQty;
